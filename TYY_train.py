...
def main():
    ...
    with open(os.path.join("models", "TYY_2stream.json"), "w") as f:
    ...
    model.save_weights(os.path.join("models", "TYY_2stream.h5"), overwrite=True)
    #pd.DataFrame(hist.history).to_hdf(os.path.join("models", "history_{}_{}.h5".format(depth, k)), "history")
    pd.DataFrame(hist.history).to_hdf(os.path.join("models", "history_TYY_2stream.h5"), "history")
